...
2. **Verificación**: los 17 puntos de `.opencode/skills/stitch-prompt-crafter/reference/checklist.md` contra el borrador (persona, capas, valores literales, prohibiciones, coincide con Fase 3, casos de borde como nodos propios, tamaño ≤4500, `modelId` y `designSystem` correctos, navegación sin ítems genéricos de Stitch, dials coherentes, estados de componentes, léxico anti-default) — se corrige el archivo antes de gastar una generación real y el resultado queda marcado en la sección `## Checklist` del borrador.
