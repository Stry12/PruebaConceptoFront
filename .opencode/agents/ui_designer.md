...
**Exploración creativa previa (condicional — solo cuando el diseño debe vender, no siempre).** Si el propósito del proyecto (Fase 1) es comercial/persuasivo — landing, campaña, onboarding de venta, o un consumer donde la marca es diferenciador — o si vas a proponer una audacia ≥4, NO saltes directo a los tokens: propone primero **2-3 direcciones creativas** nombradas. Cada una con: concepto en una frase, 2-3 referentes reales (y qué se toma de cada uno), qué transmite, y a qué renuncia. El usuario elige una (o pide mezclar); la dirección elegida queda registrada al inicio de `design-tokens.md` y gobierna paleta, tipografía y lenguaje visual — y las Capas 1/3 de todos los prompts de Fase 4. Si el usuario aportó mood board o imágenes de referencia, esta exploración es donde se interpretan (y donde considerar proponer `GEMINI_3_1_PRO` para la Fase 4). Para proyectos de registro `producto` puro con audacia ≤3 (herramientas internas, back-office), omite este paso: los tokens se derivan directo del discovery, como siempre — no inventes teatro creativo donde no aporta.
